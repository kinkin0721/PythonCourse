def ab(guess, answer):
    ...
    for index in range(len(guess)):
        if guess[index] == answer[index]:
            correct_weizhi_def += 1

    # 数字对，位置不对，的数的个数
    correct_shuzi = 0
    for jie_shuzi in guess:
        if jie_shuzi in answer:
            correct_shuzi_def += 1

    return correct_weizhi_def,  correct_shuzi_def
    ...
    for index in range(int(caijici)):
        ...
        if jie.isdigit():
            ...
            # 输入合法
                 ...
                 correct_weizhi, correct_shuzi = ab(jie, daan_wuchongfu_str)
